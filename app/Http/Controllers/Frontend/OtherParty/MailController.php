...
class MailController extends Controller {
  ...
  public function index($subdomain,$keyword =NULL){
   ...
   $manager_id = RegisteredOtherParties::where('other_party_id',Auth::user()->id)->select('manager_id')->first();
   ...
   $manager_level_other_parties=RegisteredOtherParties::where('manager_id',$manager_id['manager_id'])->pluck('other_party_id')->all();
    ...
    //dd($inbox_read_messages); 
     return view('frontend/other-party/mail-box-management/mail-box-management',compact('inbox_messages','inbox_read_messages'));
  }
  ...
  public function sentItems(){
   $manager_trash_table_messages=TrashMails::where('user_id',Auth::user()->id)->pluck('email_table_id')->all();
    ...
    return view('frontend/other-party/mail-box-management/sent-items',compact('sent_messages'));
  }
  ...
  public function getImportant(){
   ...
   return view('frontend/other-party/mail-box-management/important-messages',compact('important_messages'));
   ...
   return view('frontend/other-party/mail-box-management/trash-messages',compact('trash_messages'));
  }

  public function compose($subdomain,$keyword=NULL){
     $users="";
    $manager_id = RegisteredOtherParties::where('other_party_id',Auth::user()->id)->select('manager_id')->first();
    if($keyword == 'all'){
    ...
    // return $users;
    return view('frontend/other-party/mail-box-management/mail-compose',compact('users'));
  }
 ...
 public function messageDetailsRead($subdomain,$hashid,$hashPagename){
  ...
  // return $dummy_page_wise_numbers;
   return view('frontend/other-party/mail-box-management/read-message',compact('message_record_detail','from_mail','to_mail','dummy_page_wise_numbers','thread_reply_messages'));
  }
  ...
  public function emailData($subdomain,Request $request){
      ...
      $manager_id = RegisteredOtherParties::where('other_party_id',Auth::user()->id)->select('manager_id')->first();
       ...
       $manager__level_employees_ids = Employee::where('manager_id',$manager_id['manager_id'])->pluck('employee_id')->all();
       $manager__level_client_ids = ManagerClients::where('manager_id',$manager_id['manager_id'])->pluck('client_id')->all();
       $collection = collect([$manager__level_employees_ids,$manager__level_client_ids,$manager__level_group_ids,[$manager_id['manager_id']]]);
       $collapsed = $collection->collapse();
  ...
  public function getDetailMessageTrash($subdomain,Request $request){
   ...
   $move_to_trash = TrashMails::create([
    ...
    if($request->get('page_name') == 1 || $request->get('page_name') == 2){
       return redirect()->to('other-party/mail-box');
    }elseif($request->get('page_name') == 3){
       return redirect()->to('other-party/sent-items');
    }elseif($request->get('page_name') == 4){
       return redirect()->to('other-party/drafts');
    }elseif($request->get('page_name') == 6){
       return redirect()->to('other-party/important-message');
    }     
  }
  ...
  public function getDetailMessagePerminantDelete($subdomain,Request $request){
	   ...
	   $important_table_record_found=ImportantMails::where('email_table_id',$request->get('detail_message'))->where('user_id',Auth::user()->id)->delete();
	    return redirect()->to('other-party/trash');
  }
  ...
  public function getReplyMessage($subdomain,$hash_message_id,$hash_page_name){
    ...
    return view('frontend/other-party/mail-box-management/reply-message',compact('reply_to_message_record','message_id','page_name_dummy_number'));
  }
...
//group creation in messaging start
  public function manageGroup(){
    ...
    return view('frontend/other-party/mail-box-management/manage-group',compact('mail_group'));
  }
  ...
  public function addGroup(){
     ...
     $employees = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
      ...
      return view('frontend/other-party/mail-box-management/add-group',compact('employees','clients','other_parties'));
  }
    ...
    if ($validator->fails()){
          ...
          $request, $validator
            ...
            $record = User::where('name',$request->get('group_name'))->where('mail_group_manager_id',$manager_id['manager_id'])->first();
                 if(count($record) != 0){
                  $url = url('other-party/create-group');
                  if ($request->ajax()) {
            ...
            $url = url('other-party/manage-group');
            if ($request->ajax()){
                    return response()->json(array(
  ...
  public function editGroup($subdomain,$hashgroupid){
   ...
   // return $record['selected_ids'];
    return view('frontend/other-party/mail-box-management/edit-group',compact('employees','clients','other_parties','record','data'));
  }
           ...
           $manager_id = ManagerClients::where('client_id',Auth::user()->id)->select('manager_id')->first();
            ...
            $data= $request->all();
             if($data['group_name'] != $data['hidden_group_name']){
                 ...
                 if(count($record) != 0){
                  $url = url('other-party/edit-group');
                  if ($request->ajax()) {
                     $validator->errors()->add("login_error", "Already this group name existed");
                      return response()->json($validator->getMessageBag(), 301);
                     }else{
