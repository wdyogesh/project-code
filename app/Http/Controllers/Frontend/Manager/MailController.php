...
class MailController extends Controller {
  ...
  public function index($subdomain,$keyword =NULL){
    ...
    $manager_level_employees=Employee::where('manager_id',Auth::user()->id)->pluck('employee_id')->all();
    $manager_level_clients=ManagerClients::where('manager_id',Auth::user()->id)->pluck('client_id')->all();
    $manager_level_other_parties=RegisteredOtherParties::where('manager_id',Auth::user()->id)->pluck('other_party_id')->all();
    if($keyword == ''){
          ...
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereNotIn('emails.id',$manager_unread_table_messages)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
          //All read inbox messages with check important message query
           $inbox_read_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereIn('emails.id',$manager_unread_table_messages)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
    }elseif($keyword == 'ad'){
         //All underead inbox messages with check important message query
            $inbox_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereNotIn('emails.id',$manager_unread_table_messages)->where('to_id',Auth::user()->id)->where('from_id',1)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
          //All read inbox messages with check important message query
           $inbox_read_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereIn('emails.id',$manager_unread_table_messages)->where('to_id',Auth::user()->id)->where('from_id',1)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
    }elseif($keyword == 'em'){
          //All underead inbox messages with check important message query
          $inbox_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereNotIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_employees)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
          //All read inbox messages with check important message query
           $inbox_read_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_employees)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
    }elseif($keyword == 'cl'){
       //All underead inbox messages with check important message query
          $inbox_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereNotIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_clients)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
          //All read inbox messages with check important message query
           $inbox_read_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_clients)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
    }elseif($keyword == 'ot'){
       //All underead inbox messages with check important message query
          $inbox_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereNotIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_other_parties)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
          //All read inbox messages with check important message query
           $inbox_read_messages = Emails::leftJoin('important_emails as info', function($join){
              $join->on('info.email_table_id', '=', 'emails.id');
              $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
          })->whereNotIn('emails.id',$manager_trash_table_messages)->whereIn('emails.id',$manager_unread_table_messages)->whereIn('from_id',$manager_level_other_parties)->where('to_id',Auth::user()->id)->where('is_drafts',0)->select('*','emails.id as master_table_email_id')->get();
    }
    return view('frontend/business-manager/mail-box-management/mail-box-management',compact('inbox_messages','inbox_read_messages'));
  }
  ...
  public function sentItems(){
    ...
    $sent_messages = Emails::leftJoin('important_emails as info', function($join){
        $join->on('info.email_table_id', '=', 'emails.id');
        $join->on('info.user_id', '=', \DB::raw(Auth::user()->id));
    })->whereNotIn('emails.id',$manager_trash_table_messages)->where('from_id',Auth::user()->id)->where('manager_id',Auth::user()->id)->where('is_drafts',0)->where('master_message_id',NULL)->select('*','emails.id as master_table_email_id')->get();
    //dd($sent_messages);

    return view('frontend/business-manager/mail-box-management/sent-items',compact('sent_messages'));
  }
  ...
  public function getImportant(){
    ...
    return view('frontend/business-manager/mail-box-management/important-messages',compact('important_messages'));
   ...
   public function getTrash(){
    ...
    return view('frontend/business-manager/mail-box-management/trash-messages',compact('trash_messages'));
  }

  public function compose($subdomain,$keyword=NULL,$clientorotherparty=NULL){
     $notes_categores=NotesCategory::where('manager_id',Auth::user()->id)->where('soft_delete',0)->get();
    ...
    if($keyword == 'all'){
      ...
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
       ...
       $manager__level_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',5)->pluck('employee_id')->all();
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
     }elseif($keyword == 'em-reception'){
      $manager__level_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',6)->pluck('employee_id')->all();
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
     }elseif($keyword == 'em-operational'){
      $manager__level_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',7)->pluck('employee_id')->all();
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
     }elseif($keyword == 'em-facetoface'){
      $manager__level_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',8)->pluck('employee_id')->all();
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
      ...
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
         ...
         $manager__level_category_based_client_ids = ClientNotes::whereIn('category_name',$id)->where('manager_id',Auth::user()->id)->pluck('client_id')->all();
      ...
      $users = User::whereIn('id',$manager__level_employees_ids)->select('id','email','name','surname','role_id')->get();
       ...
       $id = Hashids::decode($clientorotherparty);
       $invited_other_party=OtherPartyInvitedUsers::whereIn('other_party_category_id',$id)->where('manager_id',Auth::user()->id)->pluck('id')->all();
       $manager__level_other_parties_ids = RegisteredOtherParties::whereIn('invitation_table_record_id',$invited_other_party)->where('manager_id',Auth::user()->id)->pluck('other_party_id')->all();
    ...
    // return $users;
    return view('frontend/business-manager/mail-box-management/mail-compose',compact('users','keyword','notes_categores','other_party_categories'));
  }
 ...
 public function messageDetailsRead($subdomain,$hashid,$hashPagename){
  ...
  // return $dummy_page_wise_numbers;
   return view('frontend/business-manager/mail-box-management/read-message',compact('message_record_detail','from_mail','to_mail','dummy_page_wise_numbers','thread_reply_messages'));
  }
  ...
  public function emailData($subdomain,Request $request){
       ...
       //$term = trim('cli');
        ...
        if($request->category == "all"){
           $manager__level_group_ids = User::where('mail_group_manager_id',Auth::user()->id)->pluck('id')->all(); 
           $manager__level_employees_ids = Employee::where('manager_id',Auth::user()->id)->pluck('employee_id')->all();
           $manager__level_client_ids = ManagerClients::where('manager_id',Auth::user()->id)->pluck('client_id')->all();
           $manager__level_otherparties_ids = RegisteredOtherParties::where('manager_id',Auth::user()->id)->pluck('other_party_id')->all();
           $collection = collect([$manager__level_employees_ids,$manager__level_client_ids,$manager__level_group_ids,$manager__level_otherparties_ids,[1]]);
           $collapsed = $collection->collapse();
          ...
          $manager__level_admin_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',5)->pluck('employee_id')->all();
          $collection = collect([$manager__level_admin_employees_ids]);
          $collapsed = $collection->collapse();
        }elseif($request->category == "em-reception"){
          $manager__level_reception_employees_ids = Employee::where('manager_id',Auth::user()->id)->where('businessmanagers_employee_roles_id',6)->pluck('employee_id')->all();
      ...
      foreach($direct_send_mail_send_users_records as $key => $value){
        ...
        //return $is_drafts;
          ...
          /* mail sending code start*/
            $manager_user_record=User::where('id',Auth::user()->id)->first();
            $to_user_record=User::where('id',$manager_record['to_id'])->first();
            $data = array('manager_name'=>$manager_user_record['name'],'manager_email'=>$manager_user_record['email'],'to_mail'=>$to_user_record['email'],'to_user_name'=>$to_user_record['name'],'to_user_surname'=>$to_user_record['surname'],'business_name' =>$subdomain,"body" => "Test mail");
            $mail_sent=Mail::send('frontend.mail-templates.ICMAIL-alert', $data, function($message) use ($data){
                        $message->to($data['to_mail'], 'Receiver')
                                  ->subject($data['business_name'].'New message alert from intell-COMM');
                        $message->from($data['manager_email'],'Sender');         
                      });
          /*mail sending code end*/            
      }
     
      $url = url('manager/mail-box');
        if ($request->ajax()) {
  ...
  public function getDetailMessageTrash($subdomain,Request $request){
   ...
   $move_to_trash = TrashMails::create([
    ...
    if($request->get('page_name') == 1 || $request->get('page_name') == 2){
       return redirect()->to('manager/mail-box');
    }elseif($request->get('page_name') == 3){
       return redirect()->to('manager/sent-items');
    }elseif($request->get('page_name') == 4){
       return redirect()->to('manager/drafts');
    }elseif($request->get('page_name') == 6){
       return redirect()->to('manager/important-message');
    }     
  }
  ...
  public function getDetailMessagePerminantDelete($subdomain,Request $request){
     ...
     $important_table_record_found=ImportantMails::where('email_table_id',$request->get('detail_message'))->where('user_id',Auth::user()->id)->delete();
      return redirect()->to('manager/trash');
  }
  ...
  public function getReplyMessage($subdomain,$hash_message_id,$hash_page_name){
    ...
    return view('frontend/business-manager/mail-box-management/reply-message',compact('reply_to_message_record','message_id','page_name_dummy_number'));
  }
...
//group creation in messaging start
  public function manageGroup(){
    $mail_group= User::join('email_group as info', 'info.group_user_id', '=', 'users.id')
                   ->where('info.manager_id',Auth::user()->id)
    ...
    return view('frontend/business-manager/mail-box-management/manage-group',compact('mail_group'));
  }
  ...
  public function addGroup(){
      ...
      return view('frontend/business-manager/mail-box-management/add-group',compact('employees','clients','other_parties'));
  }
  ...
  public function storeGroup($subdomain,Request $request){
    ...
    if ($validator->fails()){
          ...
          $request, $validator
            ...
            $record = User::where('name',$request->get('group_name'))->where('mail_group_manager_id',Auth::user()->id)->first();
                 if(count($record) != 0){
            ...
            $record = User::where('email',$mail_complete_attached_word)->where('mail_group_manager_id',Auth::user()->id)->first();
                 if(count($record) != 0){
                  $url = url('manager/create-group');
                  if ($request->ajax()) {
  ...
  public function editGroup($subdomain,$hashgroupid){
   ...
   // return $record['selected_ids'];
    return view('frontend/business-manager/mail-box-management/edit-group',compact('employees','clients','other_parties','record','data'));
  }
          ...
          $request, $validator
            ...
            $data= $request->all();
             if($data['group_name'] != $data['hidden_group_name']){
                 $record = User::where('name',$data['group_name'])->where('mail_group_manager_id',Auth::user()->id)->first();
                 if(count($record) != 0){
                  $url = url('manager/edit-group');
                  if ($request->ajax()) {
                     $validator->errors()->add("login_error", "Already this group name existed");
                      return response()->json($validator->getMessageBag(), 301);
                     }else{
