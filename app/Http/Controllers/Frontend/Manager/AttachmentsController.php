...
class AttachmentsController extends Controller {
   public function index($subdomain,$hashclienthid,$attachmentheading=NULL){
       ...
       $fileter_attachment_headings=Attachments::where('manager_id',Auth::user()->id)->where('client_id',$client_id)->get();
       if($attachmentheading == ""){
         $attachments=Attachments::where('manager_id',Auth::user()->id)->where('client_id',$client_id)->get();
       }else{
         $attachments=Attachments::where('manager_id',Auth::user()->id)->where('client_id',$client_id)->where('heading',$attachmentheading)->get();
       }
      
       $client_record= User::where('id',$client_id)->first();
      return view('frontend/business-manager/notes/attachments',compact('client_record','attachments','fileter_attachment_headings'));
  }

  public function createAttachment($subdomain,$hashclienthid){
       $client_id = Hashids::decode($hashclienthid)[0];
       $client_record= User::where('id',$client_id)->first();
      return view('frontend/business-manager/notes/add-attachment',compact('client_record'));
  }
  public function postAttachment($subdomain,Request $request){
    ...
    if ($validator->fails()){
      ...
      $url = url('manager/client-attachments/'.Hashids::encode($data['client_name']));
      if ($request->ajax()){
   ...
   public function editAttachment($subdomain,$attachmentid,$hashclienthid){
      ...
      return view('frontend/business-manager/notes/edit-attachment',compact('client_record','attachment_record'));
  }
