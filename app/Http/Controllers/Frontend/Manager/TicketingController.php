...
class TicketingController extends Controller {
	...
	public function index(){
	    $my_tickets= Ticketings::where('manager_id',Auth::user()->id)->get();
		return view('frontend/business-manager/ticket-management/my-tickets',compact('my_tickets'));
	}

    public function create(){   
		return view('frontend/business-manager/ticket-management/create-ticket');
	}
	...
	public function send(Request $request){
		...
		if ($validator->fails()){
           ...
           // return 'hai';
			$data= $request->all();           
            $dt = Carbon::now();
            $business_name= ManagerBusinessDetails::where('user_id',Auth::user()->id)->first();
		    $ticket_record = Ticketings::create([
                            	'ticket_id'=> strtoupper("BM".date('dmHis')),
                            	'business_name'=> $business_name['businesss_name'],
    ...
    public function thread($subdomain,$ticketid){
		...
		//dd($main_thread);
        return view('frontend/business-manager/ticket-management/ticket-thread',compact('main_ticket','main_thread'));
    }
    ...
    public function reply(Request $request){
		...
		if ($validator->fails()){
			...
			$url = url('manager/open-thread/'.$hash_ticket_id);
			if ($request->ajax()){
	            return response()->json(array(
	              'success' => 'Ticket Created Successfully',
	              'modal' => true,
