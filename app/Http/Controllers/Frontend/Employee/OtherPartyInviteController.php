...
class OtherPartyInviteController extends Controller {
  ...
  public function index(){ 
   $manager_record = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first();        
    $other_party_invited_users = OtherPartyInvitedUsers::where('manager_id',$manager_record['manager_id'])->where('register_by_manager',NULL)->get();
    $data=[];
    ...
    return view('frontend/employee/otherparty-invited-users-management/manage-invited-other-party-users',compact('main_other_party_invitation_sent_users'));
  }	

  public function getSendInvitation(){
      $manager_record = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first();
      $categories=OtherPartyCategory::where('manager_id',$manager_record['manager_id'])->where('soft_delete',0)->get();
    return view('frontend/employee/otherparty-invited-users-management/create-invited-other-party-users',compact('categories'));
  }	

  public function postSendInvitation(Request $request){
      $request->all();
     $messages = [
         'other_party_user_name.required' => 'Other party user name field required',
      ...
      $manager_record = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first();
		  ...
		  $business_user = User::with('businessdetails')->where('id',$manager_record['manager_id'])->first();
      ...
      $invitation_completed_manager_level=OtherPartyInvitedUsers::where('manager_id',$manager_record['manager_id'])->where('other_party_email',$data['other_party_email'])->get();
      ...
      //business manager level
        $manager_mail= $business_user['email'];
        if($manager_mail == $data['other_party_email']){
          if ($request->ajax()) {
                  $validator->errors()->add("login_error", "Sorry user already existed in your business with same email you can't send invitation");
                  return response()->json($validator->getMessageBag(), 301);
                 }else{
                  return redirect()->back()->with("fail", "Sorry user already existed in your business with same email you can't send invitation")->withInput();
                 }
        }
        //business manager alter native level
       $manager_alternative_mail_check= ManagerAlternativeAccountFindingTable::where('main_manager_id',$business_user['id'])->select('alternative_business_manager_account_user_id')->first();
      ...
      $manager_level_client_ids = ManagerClients::where('manager_id',$business_user['id'])->pluck('client_id')->all();
      ...
      $manager__level_employee_ids = Employee::where('manager_id',$business_user['id'])->pluck('employee_id')->all();
      ...
      $manager_level_otherparty_ids = RegisteredOtherParties::where('manager_id',$business_user['id'])->pluck('other_party_id')->all();
     ...
     $manaeger= User::where('id',$manager_record['manager_id'])->first();
      ...
      $mail_sent=Mail::send('frontend.mail-templates.other-party-invitation-sent', $data2, function($message) use ($data2){
        ...
        //Invitation sent through mail end
			...
			$url = url('employee/invite-other-party');
			if ($request->ajax()){
