...
class ClientNotesController extends Controller {
	...
	public function index(){ 
		...
		return view('frontend/employee/client-notes/notes-category/manage-notes-category',compact('all_records'));
	}

    public function create(){
		
	 return view('frontend/employee/client-notes/notes-category/create-category');
    }
    ...
    public function storeCategory(Request $request){
		...
		if ($validator->fails()){
           ...
           // return 'hai';
            ...
            $dt = Carbon::now();
            $manager_id = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first();
            $unique_check=NotesCategory::where('category_name',$data['category_name'])->where('manager_id',$manager_id['manager_id'])->first();
            if(count($unique_check) != 0){
            	...
            	$notes_cat=NotesCategory::where('id',$unique_check['id'])->first();
			            ...
			            $url = url('employee/manage-notes-category');
						if ($request->ajax()){
    ...
    public function editCategory($subdomain,$hashid){
    	...
    	return view('frontend/employee/client-notes/notes-category/edit-category',compact('notes_category'));
    }
    ...
    */
    public function update(Request $request){
		...
		if ($validator->fails()){
            ...
            if($data['category_name'] != $data['category_hidden_name']){
            	$unique_check=NotesCategory::where('category_name',$data['category_name'])->where('manager_id',$manager_id['manager_id'])->get();
               if(count($unique_check) != 0){
            	if ($request->ajax()) {
	                $validator->errors()->add("login_error", "Sorry! Already category existed in your business...");
	                return response()->json($validator->getMessageBag(), 301);
    ...
    public function manageNotes(){
    	$manager_id = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first();
    	$client_notes=ClientNotes::where('manager_id',$manager_id['manager_id'])->get();
    	$data=[];
    	...
    	//dd($all_client_notes);
    	return view('frontend/employee/client-notes/notes/manage-notes',compact('all_client_notes'));
    }
    ...
    public function createNotes(){
        ...
        $notes_category=NotesCategory::where('manager_id',$manager_id['manager_id'])->where('soft_delete',0)->get();
    	return view('frontend/employee/client-notes/notes/create-client-notes',compact('managers_clients','notes_category'));
    }
    ...
    public function postNotes(Request $request){
		...
		if ($validator->fails()){
			...
			$url = url('employee/manage-client-notes');
			if ($request->ajax()){
	            return response()->json(array(
  ...
  public function editNotes($subdomain,$hashid){
	 ...
	 // return $data;
    	return view('frontend/employee/client-notes/notes/edit-notes',compact('managers_clients','notes_category','notes'));
   }
   ...
   public function updateNotes(Request $request){
    ...
    public function notesDetails($subdomain,$hashid){
    	...
    	$client_record=User::where('id',$notes['client_id'])->select('name','surname')->first();
		  ...
		  $data['category_name']=$category_record['category_name'];
		  $data['notes']=$notes['notes'];
		  $business_employee=User::where('id',$notes['common_user_id'])->first();
	      $data['added_by']=$business_employee['name'].' '.$business_employee['surname'];
	      $roles=Role::where('id',$business_employee->role_id)->select('role_name')->first();
	      $data['role_name']=$roles['role_name'];
	      $data['file_name']=$notes['file_name'];
	      $data['created_record_date']=$notes['created_record_date'];
	      $data['updated_record_date']=$notes['updated_record_date'];
	 // return $data;
    	return view('frontend/employee/client-notes/notes/detail-notes',compact('data'));
  }  
