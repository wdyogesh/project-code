...
class OtherPartyController extends Controller {
  ...
  public function index(){
     ...
     $managers_registered_other_party_users= User::join('user_addresses as info', 'info.user_id', '=', 'users.id')
                   ...
                   ->where('role_id',5)
                   ->where('registered_other_parties.manager_id',$manager_record['manager_id'])
   ...
   return view('frontend/employee/otherparty-management/manage-otherparty',compact('main_other_party_register_users','manager_record'));
  }	 
     ...
     $managers_registered_other_party_users= User::join('user_addresses as info', 'info.user_id', '=', 'users.id')
      ...
      //dd($managers_registered_other_party_users);
        ...
        foreach ($managers_registered_other_party_users as $key => $managers_registered_other_party_user) {     
          ...
          //dd($main_other_party_register_users);
   return view('frontend/employee/otherparty-management/details-otherparty',compact('main_other_party_register_users'));
  }
  ...
  public function otherPartyRegistration(){
    ...
    $manager_record = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first(); 
    $other_party_category=OtherPartyCategory::where('manager_id',$manager_record['manager_id'])->where('soft_delete',0)->get();
    return view('frontend/employee/otherparty-management/otherparty-registration',compact('countries','other_party_category'));
  }
  ...
  public function store(Request $request){
    ...
    if ($validator->fails()){
      ...
      //business manager level
        $manager_record = Employee::where('employee_id',Auth::user()->id)->select('manager_id')->first(); 
        $business_manager_record= User::where('id',$manager_record['manager_id'])->first();
      ...
      $othe_party_invitation_store = OtherPartyInvitedUsers::create([
                          ...
                          'register_by_manager'=>$manager_record['manager_id'],
                          'register_by_employee'=>Auth::user()->id,
      ...
      //mail sent to set security questions and password end
        ...
        $url = url('employee/manage-other-parties');
        if ($request->ajax()){
