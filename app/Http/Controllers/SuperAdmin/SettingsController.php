...
class SettingsController extends Controller {
	...
	public function profile(){ 
         $user_profile = User::with('useraddresses')->whereId(Auth::user()->id)->first();
         $user_role=Role::where('id',$user_profile['role_id'])->select('role_name')->first();
		return view('admin/settings/profile',compact('user_profile','user_role'));
	}

	public function editProfile($hashid){ 
        $manager_id = Hashids::decode($hashid)[0];
        $user_profile = User::whereId($manager_id)->first();
          //dd($user_profile);    
       ...
       return view('admin/settings/edit-profile',compact('user_profile','countries','selected_countery_all_states_foreah','selected_state_all_cities_foreah'));
	}
