			...
			<div class="box">
			 ...
			 <div class="box-body">
				<table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
					<?php $i=1;?>
						@foreach($managers_employees as $managers_employee)
						<tr>
							<td>{{$i++}}</td>
							<td>{{ucfirst($managers_employee->name)}} {{ucfirst($managers_employee->surname)}}</td>
							<td>{{$managers_employee->email}}</td>
							<td>{{$managers_employee->country}}</td>
							<td>{{$managers_employee->registration_id}}</td>
							<td>{{$managers_employee->phone_number}}</td>
							<td>{{$managers_employee->employee_role_name}}</td>
							<td>{{$managers_employee->main_user_table_created_record_date}}</td>
							@if($managers_employee->main_user_table_updated_record_date == "")
							<td>--</td>
							@else
							<td>{{$managers_employee->main_user_table_updated_record_date}}</td>
							@endif

							
							<td>
							<button class="btn btn-warning btn-detail open_modal" value="{{$managers_employee->main_user_id}}" title="Details"><span class="glyphicon glyphicon-folder-open"></span></button>
                            
							 <a href="{{url('manager/edit-employee/'.Hashids::encode($managers_employee->main_user_id))}}" class="btn btn-info" title="Edit">
								 <span class="glyphicon glyphicon-pencil"></span> 
							 </a>
							 <button class="btn btn-danger btn-delete delete-employee" value="{{$managers_employee->main_user_id}}" title="Delete"><span class="glyphicon glyphicon-trash"></span></button>
...
<script>
var url = "{{url('manager/delete-employee')}}";
