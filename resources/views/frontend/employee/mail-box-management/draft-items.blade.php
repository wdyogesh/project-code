            ...
            <div class="box-body no-padding">
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  <tbody>
                  @foreach($draft_messages as $draft_message)
                  <tr>
                   <td>
                   <input type="checkbox" name="selected[]" value="{{$draft_message->master_table_email_id}}">
                   </td>
                    <td>
                     <a href="{{url('manager/important',$draft_message->master_table_email_id)}}"><i style="color:#999" class="fa fa-star @if($draft_message->email_table_id != "") text-yellow @endif"></i></a>
                    </td>
                    <td class="mailbox-name"><a href="{{url('manager/read-message-details/'.Hashids::encode($draft_message->master_table_email_id).'/'. Hashids::encode(4) )}}" title="Subject">{{$draft_message->subject}}</a></td>

                     

                    <td class="mailbox-subject"><b title="Message">{!! str_limit($draft_message->message,120) !!}</b>
                    </td>
                    @if(!empty($draft_message->attachments))
                    <td class="mailbox-attachment"><i class="fa fa-paperclip"></i></td>
                    @else
                    <td class="mailbox-attachment"></i></td>
                    @endif
                    <td class="mailbox-date">{{$draft_message->date}}</td>
