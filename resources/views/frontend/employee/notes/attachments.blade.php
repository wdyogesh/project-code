 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper manager-content-wrapper">
    ...
    <section class="content-header">
      <h1>
       {{$client_record['name']}} {{$client_record['surname']}} / Attachments
      </h1>
     <a href="{{url('employee/attach-file/'.Hashids::encode($client_record['id']))}}" class="btn btn-info pull-right">Add</a>
    </section>
          ...
          <div class="box box-primary">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu scrollable-menu" role="menu">
                 <li><a href="{{url('employee/client-attachments/'.Hashids::encode($client_record['id']))}}">All</a></li>
                @if(isset($attachments))
               @foreach($fileter_attachment_headings as $attachment)

                    <li><a href="{{url('employee/client-attachments/'.Hashids::encode($client_record['id']).'/'.$attachment['heading'])}}">{{$attachment['heading']}}</a></li>
               @endforeach
               @endif
                </ul>
            </div> 
            ...
            <tr>
              ...
              <td>
                 @if($attachment['common_user_id'] == Auth::user()->id)
                  <a href="{{url('employee/edit-attachment/'.Hashids::encode($attachment['id']).'/'.Hashids::encode($client_record['id']))}}" class="btn btn-info">
                   Edit
                 </a>
...
<script src="{{asset('js/login-bootstrap-modal-form.js')}}"></script>
<script>
var url = "{{url('employee/delete-attachment')}}";
