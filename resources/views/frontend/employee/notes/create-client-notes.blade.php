...
@section('content')
 ...
 -->
     ...
     <form action="{{url('employee/create-notes')}}" class="-bootstrap-modal-form form-horizontal" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="appointments">
                    ...
                    @foreach($apointment_details as $appointment)
                      <option value="{{$appointment['appointment_id']}}">{{$appointment['appointment_date']}}[{{$appointment['client_name']}}]</option>
                    @endforeach
                    @endif  
                    </select>  
